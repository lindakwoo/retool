...
reason_counts AS (
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN syn.appointment_id IS NOT NULL
      THEN ab.appointment_id END)                                                AS scribed_appointments
  FROM appt_base ab
...
SELECT
  reason,
  total_appointments,
  scribed_appointments,
  scribed_appointments::numeric
    / NULLIF(total_appointments, 0)                                              AS pct_scribed,
  total_appointments::numeric
    / NULLIF((SELECT total FROM total_appts), 0)                                 AS pct_of_total_appointments
FROM reason_counts
ORDER BY pct_scribed DESC