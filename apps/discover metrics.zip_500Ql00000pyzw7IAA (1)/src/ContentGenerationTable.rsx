...
>
  <Header>
    <Text
      id="contentGenerationRunsTitle"
      _disclosedFields={{ array: [] }}
      value="#### Content Generation runs"
    ...
    <ToggleButton
      ...
      value="{{ ContentGenerationTable.showBody }}"
    >
      <Event
        event="change"
        method="setShowBody"
        params={{ ordered: [{ showBody: "{{ self.value }}" }] }}
        pluginId="ContentGenerationTable"
        type="widget"
        waitMs="0"
        waitType="debounce"
      />
  ...
  <View id="dc1f9" viewKey="View 1">
    <Text
      id="ContentGenerationSelectedRunTitle"
      _disclosedFields={{ array: [] }}
      value="**Selected Run:** [{{content_generation_table.selectedRow.data.runId}}]({{content_generation_table.selectedRow.data.link}})"
      verticalAlign="center"
    />
    <TableLegacy
      id="content_generation_table"
      _columns={[
        "pipelineName",
        "link",
        "org_id",
      ...
      data="{{ dagster_get_content_gen_jobs.data }}"
    ...
    />
    <JSONEditor
      id="contentGenerationRunInsight"
      value="{{content_generation_table?.selectedRow?.data?.runConfig}}"
    />
    <Container
      id="contentGenerationLogsContainer"
      footerPadding="4px 12px"
      headerPadding="4px 12px"
      padding="12px"
      showBody={true}
    ...
    >
      <Header>
        <Text
          id="contentGenerationRunLogsTitle"
          _disclosedFields={{ array: [] }}
          value="#### Run logs"
          verticalAlign="center"
        />
      ...
      <View id="d46c1" viewKey="View 1">
        <Text
          id="contentGenerationRunLogsOutput"
          _disclosedFields={{ array: [] }}
          heightType="fixed"
          value="{{content_generation_logs.data}}"
          verticalAlign="center"
        />
      </View>
    </Container>
