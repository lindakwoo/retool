...
tableOrders.selectedRows.forEach((row) => {
  ...
  if(order){
    if (currencyPostageDueRefund.value>0) {
      order.legacyRefundRequest = {
        author_email: current_user.email,
        refund_reason: selectPostageDueReason.value,
        notes: textAreaPostageDueNotes.value,
        is_full_refund: false,
        legacy_refund_amount_cents:currencyPostageDueRefund.value*100,
