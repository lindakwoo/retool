...
>
  <Header>
    <Tabs
      id="batchTab2"
      disabled="{{ 
  //CurrentUserObj?.value?.user_role.length === 0 ? true : false
false}}"
      itemMode="static"
      navigateContainer={true}
      targetContainerId="batchProcessContainer2"
      value="{{ self.values[0] }}"
    >
  ...
  <View id="d2a01" label="Batch Process" viewKey="batchProcess">
    <FileInput
      ...
      label=""
      labelPosition="top"
      loading="{{browseExcelFile2.parsing}}"
      parseFiles={true}
      style={{ ordered: [] }}
      textBefore="Browse"
    >
    ...
    <Button
      id="uploadFileBtn2"
      disabled="{{(UploadBtnControl.value || browseExcelFile2.value.length === 0) && ((!(CurrentUserObj?.value?.user_role?.includes('admin')) || !(CurrentUserObj?.value?.user_role?.includes('data_analyst')) ||!(CurrentUserObj?.value?.user_role?.includes('super_admin'))) ? true: false)  }}"
      text="Upload"
    >
      <Event
        event="click"
    ...
    >
        ...
        <Event
          ...
          params={{
            ordered: [
              { fileType: "xlsx" },
              {
                data: "{{batchProcessTable2.data.length !== 0  && batchProcessTable2.data}}",
              },
      ...
      >
        <Event
          ...
          method="run"
          params={{
            ordered: [{ src: "batchProcessTable2.clearFilterStack();" }],
          }}
          pluginId=""
          type="script"
          waitMs="0"
      ...
      <Event
        ...
        params={{
          ordered: [
            {
              src: "if (!UploadBtnControl.value) {\n  batchProcessTable2.clearSelection();\n} ",
            },
          ],
        }}
        pluginId=""
  ...
  <View id="956b0" label="Batch History" viewKey="batchHistory">
    ...
    <Include src="./batchHistorySearchContainer2.rsx" />
    <Table
      id="historyBatchTable2"
      cellSelection="none"
      clearChangesetOnSave={true}
