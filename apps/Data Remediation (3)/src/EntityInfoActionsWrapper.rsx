 ...
 ValidateBvdId.data || ValidateOrbisId.data ||(CompanyNames.value.filter((name)=> name.trim() !== '').length === 0 && newCompanyName.value.trim() === '') || !companyLocalErrorMsg.hidden 
    ...
    >
      <Option
        id="65df5"
        disabled="{{ 
  !(CurrentUserObj.value.user_role.includes('admin') || CurrentUserObj.value.user_role.includes('data_analyst') || CurrentUserObj.value.user_role.includes('super_admin')) ||  lockEntity.value ||
  kompanyApiRecordData.value[0]?.kompany_api !== 'enable' 
}}
"
